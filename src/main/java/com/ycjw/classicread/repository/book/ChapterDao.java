...
public interface ChapterDao extends JpaRepository<Chapter,String> {
    ...
    @Query(value = "select chapter.chapterName from Chapter chapter where chapter.bookId=?1")
    List<String> findAllNameByChapterId(String bookId);

    @Query(value = "select chapter.chapterId from Chapter chapter where chapter.bookId=?1")
